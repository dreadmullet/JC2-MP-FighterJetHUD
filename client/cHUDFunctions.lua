...
function HUD:DrawLineHUD(pos1 , pos2 , noRoll)
	...
	if noRoll then
		...
		pos1 = GUIUtil.Rot(pos1 , Vector2(0,0) , roll)
		pos2 = GUIUtil.Rot(pos2 , Vector2(0,0) , roll)
	end
...
function HUD:DrawCircleHUD(radius , center , numSegments)
	...
	for n = 1 , numSegments do
		...
		pos = GUIUtil.Rot(pos , Vector2(0,0) , (n / numSegments) * math.tau)
		pos = pos + center
...
function HUD:DrawTextHUD(text , pos , align , noRoll)
	...
	local roll = self:GetRoll()
	if noRoll then
		pos = GUIUtil.Rot(pos , Vector2(0,0) , roll)
	end
